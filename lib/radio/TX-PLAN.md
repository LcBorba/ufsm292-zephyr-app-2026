...
1. Build the sensor image in raw mode; boot; confirm `rf2xx` probe logs the part
   ...
   For bench bring-up the `radio-tx-testbed` / `radio-rx-testbed` apps play
   these roles: TX testbed transmits, RX testbed (promiscuous) receives,
   both blink LED0 per frame.
2. Start with `IEEE802154_TX_MODE_DIRECT` and a 1 s period; confirm on a second
   ...
   The `radio-rx-testbed` image is that second board: it logs the decoded
   fields and blinks LED0 per valid frame.
