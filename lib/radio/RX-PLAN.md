...
| Custom L2 glue | `lib/radio/gateway_l2.c` | `net_if`, `net_pkt`, decoder, table, `gpio` (RX LED) |
...
- `recv` blinks the RX activity LED on every successfully stored frame (see
  §6.4): toggle `led0` (`DT_ALIAS(led0)`) via `gpio_pin_toggle_dt()`. Blink
  on store-success only, so the LED means "valid sensor data", not just
  RF energy. The toggle is non-blocking and safe from the rf2xx RX thread.
...
### 6.4 RX activity LED (packet blink)

Seeing data on hardware must not require a log cable: every received packet
blinks the on-board LED.

- LED: `led0` alias from `samr21_xpro.dts` (LED0 / PA19, active-low). No
  overlay change needed; `gateway_l2.c` uses
  `GPIO_DT_SPEC_GET(DT_ALIAS(led0), gpios)`.
- Init (once, at `radio_start()` time or first `recv`): `gpio_pin_configure_dt`
  as output-inactive. Fail-open if the alias is missing — log once, keep
  receiving.
- Blink: `gpio_pin_toggle_dt()` on each `sensor_table_store()` success.
  At the ~1 Hz sensor rate this reads as one blink per packet; under burst
  traffic it just flickers faster, which is fine for v1.
- Cost: one `gpio_dt_spec` in `.bss`, no thread, no timer, no extra stack.
  Needs `CONFIG_GPIO=y` in `gateway/prj.conf` (already implied by most
  samr21_xpro builds).
- Test: R2 bring-up checks the blink by eye; no host-side unit test (GPIO is
  hardware). The LED code lives in `lib/radio/radio_testbed.c` (shared by the
  raw-mode testbed images), and the gateway L2 reuses the same shape.
...
- [ ] `gateway/prj.conf`: `CONFIG_NETWORKING=y`,
      ...
      `CONFIG_IEEE802154_NET_IF_NO_AUTO_START=y`, `CONFIG_GPIO=y` (RX LED,
      §6.4). Keep the Ethernet options.
...
- [ ] Receive frames from the sensor image (`TX-PLAN.md` T1/T2 — use the
      `radio-tx-testbed` image on the peer board); log each decoded
      reading + RSSI/LQI and blink LED0 once per stored packet (§6.4).
...
**Exit criteria:** sensor frame → decoded `node_id`/fields in the log **plus a
LED0 blink** on real hardware; no dropped valid frames over a sustained run;
RSSI plausible.
